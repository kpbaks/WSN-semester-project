...
int main(int argc, char **argv) {
	...
	// with PKCS#7 padding
	// if fsize is a multiple of BLOCK_SIZE, then we need to add an extra block of size BLOCK_SIZE
	// to the end of the file, and pad it with PKCS#7 padding
	...
	aes_128_ecb_encrypt(key, tux, encrypted_tux, padded_fsize);

	// write encrypted ./tux.png to ./encrypted_ecb_tux.png
	

	// prepend "encrypted_ecb_" to the filepath
	char* encrypted_filepath_prefix = "encrypted_ecb_";
	char* encrypted_filepath = malloc(strlen(encrypted_filepath_prefix) + strlen(filepath) + 1);
	...
	aes_128_ecb_decrypt(key, encrypted_tux, decrypted_tux, padded_fsize);
	...
	char* decrypted_filepath_prefix = "decrypted_ecb_";
	char* decrypted_filepath = malloc(strlen(decrypted_filepath_prefix) + strlen(filepath) + 1);
	assert(decrypted_filepath != NULL);
	strcpy(decrypted_filepath, decrypted_filepath_prefix);
	strcat(decrypted_filepath, filepath);


	// write decrypted decrypted_tux to ./decrypted_ecb_tux.png
	...
	printf("%s and %s are the same (AES-128-ECB)\n", filepath, decrypted_filepath);
