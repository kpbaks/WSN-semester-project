...
uint8_t get_sbox_value(uint8_t num) { return sbox[num]; }

uint8_t get_rsbox_value(uint8_t num) { return rsbox[num]; }
...
uint8_t get_rcon_value(uint8_t num) { return rcon[num]; }
...
// used to generate the key schedule from the encryption key
// takes a four-byte input word and applies the core schedule to it,
// overwriting the input with the output.
// word is an char array of size 4 (32 bit)
// iteration is a char (8 bit)
void core(uint8_t *word, uint8_t iteration) {
...
// expand the key into an 176, 208, 240 bytes key schedule
// key is a char array of size 16, 24 or 32 (128, 192 or 256 bits)
// expandedKey is a char array of size 176, 208, 240 (11, 13, 15 rounds)
// void expand_key(uint8_t *key, uint8_t *expanded_key) {
//   // the current expanded keySize, in bytes
//   uint16_t currentSize = 0;
//   uint8_t rconIteration = 1;
//   uint8_t t[4]; // temporary 4-byte variable
...
//   // encrypt data with key using ecb mode

//   // iterate through data in block sizes of AES_BLOCK_SIZE (16) bytes
//   // encrypt each block with key
//   // store encrypted block in data
//   // repeat until all data is encrypted
// }
